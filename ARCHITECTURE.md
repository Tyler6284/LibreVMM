# VirtualBox Extended Edition — Architecture Overview

## Project Goals

VirtualBox Extended Edition aims to enhance the core VirtualBox virtualization platform with additional features and compatibility from other open-source projects, including:
...
```
Projects/VirtualBox-Community-Edition/
...
All source repositories are located in the `Cloned-Repos` directory:

| Repo | Role | Notes |
|------|------|-------|
| `Cloned-Repos/virtualbox/` | Core foundation | kBuild-based; `src/` is the main source tree |
| `Cloned-Repos/qemu/` | TCG engine + device import | Meson-based; `accel/tcg/`, `hw/i386/`, `target/i386/`, `audio/` are primary targets |
| `Cloned-Repos/86Box/` | Legacy device import | CMake + vcpkg; `src/` contains all device subsystems |
| `Cloned-Repos/Bochs/bochs/` | BIOS + device reference | Autoconf; `bios/` contains BIOS and VGABIOS ROMs |
| `Cloned-Repos/seabios/` | Firmware option | Kconfig/Makefile; `src/` is firmware source |
| `Cloned-Repos/openbios/` | Firmware option | Makefile/Kconfig; `arch/amd64/` is the x86 target |
| `Cloned-Repos/FEX/` | ARM64 Linux host translation | CMake; `CodeEmitter/` is the ARM64 code emitter |
| `Cloned-Repos/kvm-guest-drivers-windows/` | virtio Windows guest drivers | Pre-built virtio driver binaries |
| `Cloned-Repos/roms/` | ROM image library | 86Box ROM collection |
