# VirtualBox Extended Edition — Device Porting Guide

This guide provides step-by-step instructions for importing a device from QEMU, 86Box, or Bochs into VirtualBox Extended Edition using the Device Abstraction Layer (DAL).
...
1. Locate the device source files in `Cloned-Repos/qemu/hw/i386/`
...
1. Locate the device source files in `Cloned-Repos/86Box/src/`
...
4. Adapt the device to use VirtualBox's memory and I/O systems
5. Use ROM images from `Cloned-Repos/roms/` if needed

### Bochs Device Import

1. Locate the device source files in `Cloned-Repos/Bochs/bochs/`
...
1. Build VirtualBox Extended Edition
2. Create a test VM
...
After completing this guide, you should have a working device in VirtualBox Extended Edition. You can then:
