...
- [ ] Implement a device capability flags system (e.g., `DEVICE_CAP_3D`, `DEVICE_CAP_LEGACY_ISA`, `DEVICE_CAP_USB3`, `DEVICE_CAP_MCA`, `DEVICE_CAP_VIRTIO`) for UI filtering and runtime validation.
...
- [ ] **Oracle trademark compliance:** Never use the name "VirtualBox" or "Oracle VM VirtualBox" as the product name, application title, window title, installer name, or package name. All user-facing strings must use "LibreVMM." The exception is factual historical attribution (e.g., "forked from VirtualBox"). Cross-reference with Priority 5.8 for the mandatory disclaimer wording in README and About dialog.
